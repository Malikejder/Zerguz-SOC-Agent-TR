...
def write_alert_json(alert):
    with json_lock:
        try:
            ...
            print(Fore.RED + f"[!] JSON yazma hatası: {e}")
...
def raise_network_alert(attack_type, src_ip, dst_ip, details):
    ...
    print(Fore.RED + Style.BRIGHT + f"[ALARM - NIDS] {attack_type}")
    print(Fore.RED + Style.BRIGHT + f"    Kaynak IP : {src_ip}")
    print(Fore.RED + Style.BRIGHT + f"    Hedef IP  : {dst_ip}")
    print(Fore.RED + Style.BRIGHT + f"    Zaman     : {alert['timestamp']}")
    print(Fore.RED + Style.BRIGHT + f"    Detay     : {details}")
    if pcap_status == pcap_filename:
        print(Fore.GREEN + f"    [+] PCAP Kanıtı Kaydedildi: {os.path.basename(pcap_filename)}")
    print(Fore.RED + Style.BRIGHT + "=" * 60)
...
def raise_process_alert(attack_type, proc_name, pid, parent_name, details):
    ...
    print(Fore.RED + Style.BRIGHT + "=" * 60)
    print(Fore.RED + Style.BRIGHT + f"[ALARM - EDR] {attack_type}")
    print(Fore.YELLOW + f"    Süreç Adı  : {proc_name} (PID: {pid})")
    print(Fore.YELLOW + f"    Ebeveyn    : {parent_name}")
    print(Fore.YELLOW + f"    Zaman      : {alert['timestamp']}")
    print(Fore.YELLOW + f"    Detay      : {details}")
    print(Fore.RED + Style.BRIGHT + "=" * 60)
    write_alert_json(alert)
...
def check_port_scan(src_ip, dst_ip, port, proto):
    ...
    if should_alert:
        details = f"{count} farklı port {PORT_SCAN_WINDOW_SECONDS}s içinde tarandı ({proto})"
...
def check_icmp_flood(src_ip, dst_ip):
    ...
    if should_alert:
        details = f"{count} ICMP paketi {ICMP_WINDOW_SECONDS}s içinde tespit edildi (Ping Sweep/Flood şüphesi)"
    ...
    with state_lock:
        if len(qname) > DNS_MAX_LEN:
            if cooldown_ok(("DNS_LEN", src_ip), now):
                should_alert = True
                details = f"Aşırı uzun DNS sorgusu ({len(qname)} karakter): {qname[:40]}..."
        ...
        if not should_alert:
            ...
            if count > DNS_THRESHOLD and cooldown_ok(("DNS_FREQ", src_ip), now):
                should_alert = True
                details = f"Yüksek frekansta DNS sorgusu ({count} sorgu / {DNS_WINDOW_SECONDS}s)"
        ...
        for proc in psutil.process_iter(['pid', 'name', 'exe']):
            try:
                ...
                if exe:
                    ...
                    for s_dir in SUSPICIOUS_DIRS:
                        if s_dir in exe_lower:
                            ...
                            details = f"Süreç şüpheli bir dizinden çalıştırılıyor: {exe}"
                ...
                if name and name.lower() in SHELLS and parent_name.lower() in WEB_SERVERS:
                    with state_lock:
                        alerted_pids.add(pid)
                    details = f"Web sunucusu ({parent_name}) tarafından komut satırı aracı ({name}) başlatıldı!"
...
def main():
    ...
    print(Fore.GREEN + "[*] Zerguz-SOCAgent v1.1 başlatıldı...")
    print(Fore.GREEN + f"[*] NIDS Filtresi       : {BPF_FILTER}")
    print(Fore.GREEN + f"[*] EDR İzleme Durumu   : Canlı Süreç Denetimi Aktif")
    print(Fore.GREEN + f"[*] Alarm Kayıt Dosyası : {ALERTS_JSON_PATH}")
    print(Fore.CYAN + "-" * 60)
    ...
    try:
        ...
        print(Fore.RED + "[!] Bu araç root/yönetici yetkisi gerektirir.")
        sys.exit(1)
    except KeyboardInterrupt:
        print(Fore.YELLOW + "\n[*] Zerguz-SOCAgent durduruldu.")
        sys.exit(0)
